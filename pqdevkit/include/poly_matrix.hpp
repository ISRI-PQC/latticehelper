...
namespace pqdevkit
{
    ...
    public:
        PolyMatrix(const std::initializer_list<std::initializer_list<PolyProxy<_degree, _coeff_modulus>>> &other);
        PolyMatrix(const std::initializer_list<std::initializer_list<std::initializer_list<typename PolyProxy<_degree, _coeff_modulus>::coeff_type>>> &other);
        ...
        PolyMatrix scale(const typename PolyProxy<_degree, _coeff_modulus>::coeff_type &scalar) const;
        PolyMatrix scale(const typename PolyProxy<_degree, _coeff_modulus>::poly_type &poly) const;
        PolyMatrix operator+(const PolyMatrix &other) const;
        PolyMatrix operator-(const PolyMatrix &other) const;
        PolyMatrix operator|(const PolyMatrix &other) const;
        PolyMatrix operator/(const PolyMatrix &other) const;
        PolyMatrix operator*(const PolyMatrix &other) const;
        PolyVector<_degree, _coeff_modulus> operator*(const PolyVector<_degree, _coeff_modulus> &other) const;
        PolyMatrix operator*(const typename PolyProxy<_degree, _coeff_modulus>::coeff_type &scalar) const;
