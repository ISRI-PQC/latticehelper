...
namespace pqdevkit
{
    ...
    if(rows() != other.rows() || cols() != other.cols())
      {
        throw std::runtime_error(
          "PolyMatrix::operator+: PolyMatrices must have the same dimensions");
      }
  ...
  PolyMatrix PolyMatrix::operator|(const PolyMatrix &other) const
  {
    if(rows() != other.rows())
      {
        throw std::runtime_error("PolyMatrix::operator|: PolyMatrices must "
  ...
  PolyMatrix PolyMatrix::operator*(const PolyMatrix &other) const
  {
    if(cols() != other.rows())
      {
        throw std::runtime_error(
          "PolyMatrix::operator*: Number of columns in the first PolyMatrix "
          "must be equal to the number of rows in the second PolyMatrix");
      }
        ...
        for(size_t j = 0; j < other.cols(); j++)
          {
            ...
            for(size_t k = 0; k < cols(); k++)
              {
                current = current
                          + (this->poly_matrix[i].get_vector()[k].get_poly()
                             * other.transposed()
                                 .get_matrix()[k]
                                 .get_vector()[j]
                                 .get_poly());
